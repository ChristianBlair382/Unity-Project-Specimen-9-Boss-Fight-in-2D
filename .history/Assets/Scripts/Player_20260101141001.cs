...
public class Player : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Handle Attacking
        if(Input.GetMouseButtonDown(0) && (interactionState != PlayerInteractionState.Attacking || interactionState != PlayerInteractionState.Jumping) && physicalState == PlayerPhysicalState.Grounded)
        {
    ...
    private void MoveHorizontal()
    {
        ...
        else
        {
            if(rb.velocity.x <= 0.001f && rb.velocity.x >= -0.001f)
                rb.velocity = new Vector2(0, rb.velocity.y);interactionState = PlayerInteractionState.Idle;
