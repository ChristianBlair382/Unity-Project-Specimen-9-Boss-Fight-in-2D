...
public class Specimen_9 : MonoBehaviour
{
    ...
    private WellnessState currentWellness = WellnessState.Invulnerableulnerable;
    private int HP = 50;
        ...
        } else
        {
            ...
            if (stateTimer <= 0)
            {
                if (currentState == InteractionState.Waiting)
                {
                    ...
                    StartCoroutine(PerformAttack(Random.Range(0, 5)));
                }
    ...
    {
        if(collision.tag == "Player" && currentState != InteractionState.Stunned)
        {
    ...
    private void Dead()
    {
        // Specimen 9 death logic
        animator.SetTrigger("dead");
        // Disable collider to prevent further interactions
        bc.enabled = false;
        // Destroy Specimen 9 after death animation
