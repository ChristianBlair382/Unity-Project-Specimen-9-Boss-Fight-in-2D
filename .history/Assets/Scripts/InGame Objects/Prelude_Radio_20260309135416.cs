    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if(collision.CompareTag("Player"))
        {
            ...
            else
            {
                ...
                {
                    interactPromptInstance = Instantiate(interactPromptPrefab, collision.transform.position + new Vector3(0, 1.5f, -0.5f), Quaternion.identity);
                    //Lock the interact prompt's rotation so that it doesn't flip when the player turns around
                    interactPromptInstance.transform.rotation = collision.transform.rotation;
