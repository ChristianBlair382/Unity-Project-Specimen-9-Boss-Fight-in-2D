...
public class Prelude_Radio : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if(collision.CompareTag("Player"))
        {
            ...
            {
                //Check if the player is grounded befor
                if(playerScript.interactionState == PlayerInteractionState.Idle || playerScript.interactionState == PlayerInteractionState.Walking)
                {
