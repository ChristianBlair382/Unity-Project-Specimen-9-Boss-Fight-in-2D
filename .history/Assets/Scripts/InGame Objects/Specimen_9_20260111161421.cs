...
public class Specimen_9 : MonoBehaviour
{
    ...
    private IEnumerator BodyPillarAttack()
    {
        for(int i = 0; i < 3; i++)
        {
            float pillarSpawnX = Random.Range(leftBound + 1, rightBound - 1);
            Instantiate(bodyPillarPrefab, new Vector3(player.transform.position.x, -3.5f, -1f), Quaternion.identity);
    ...
    {
        if(collision.tag == "Player" && currentState != Specimen9State.Stunned)
        {
