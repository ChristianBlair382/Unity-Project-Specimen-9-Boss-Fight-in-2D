...
public class BodyPillar : MonoBehaviour
{
    ...
    private void Rise()
    {
        ...
        {
            sprite.enabled = true;
            //Upon calling Rise, have all sprites stretched horizontally to indicate emergence
            StartCoroutine(StretchPillar(new Vector2(0.1f, 1.0f), new Vector2(1.0f, 1.0f), 0.5f));
        }
    }
    private IEnumerator StretchPillar(Vector2 startScale, Vector2 targetScale, float duration)
    {
        float elapsedTime = 0f;
        float stretchDuration = duration; // Duration of the expansion effect
        while (elapsedTime < stretchDuration)
        {
            foreach (SpriteRenderer sprite in sprites)
            {
                sprite.transform.localScale = Vector2.Lerp(
                    ...
                    elapsedTime / stretchDuration); // Time factor (0 to 1)
            }
    ...
    public IEnumerator SpawnBodyPillar()
    {
        dangerZoneInstance = Instantiate(
            dangerZonePrefab, 
            new Vector3(transform.position.x, transform.position.y + 1.0f, -2.1f), 
            Quaternion.identity);
        ...
        yield return new WaitForSeconds(waitDuration); // Wait before rising
        Destroy(dangerZoneInstance);
        Rise();
        yield return new WaitForSeconds(0.5f); // Wait for pillar to fully rise
        col.enabled = true;
        yield return new WaitForSeconds(lifetime); // Pillar active duration
        col.enabled = false;
        StartCoroutine(StretchPillar(new Vector2(1.0f, 1.0f), new Vector2(0.1f, 1.0f), 0.5f));
        yield return new WaitForSeconds(0.5f); // Wait for pillar to fully retract
        Destroy(gameObject);
    }
}