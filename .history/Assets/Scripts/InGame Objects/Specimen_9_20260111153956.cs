...
public class Specimen_9 : MonoBehaviour
{
    ...
    private IEnumerator CeilingDropAttack()
    {
        ...
        if(player.transform.position.x < leftBound + 3 || player.transform.position.x > rightBound - 3)
        {
            //If player is too close to the edge, limit projectile spawn range
        }
        if(player.transform.position.x < leftBound + 3)
        {
            player.transform.position = new Vector3(leftBound + 3, player.transform.position.y, player.transform.position.z);
        }
        float projectileSpawnX = Random.Range(player.transform.position.x - 3, player.transform.position.x + 3);
        for (int i = 0; i < 3; i++)
        {
