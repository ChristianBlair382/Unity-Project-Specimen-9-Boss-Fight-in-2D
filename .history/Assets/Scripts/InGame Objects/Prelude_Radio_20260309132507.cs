    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if(collision.CompareTag("Player"))
        {
            ...
            else
            {
                ...
                {
                    interactPromptInstance = Instantiate(interactPromptPrefab, transform.position + new Vector3(0, 0.75f, 0), Quaternion.identity);
                    interactPromptInstance.transform.SetParent(collision);
