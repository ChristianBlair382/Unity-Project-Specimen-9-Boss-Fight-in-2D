...
{
    private int[] attackSequence = { 1, 2, 3, 4 }; // Define your specific attack sequence here
    ...
    {
        foreach (int attackType in attackSequence)
        {
            yield return StartCoroutine(PerformAttack(attackType));
            yield return new WaitForSeconds(5.0f); // Adjust wait time as needed
        }
    ...
    private IEnumerator RecoveryTransition(float duration)
    {
        ...
        targetPos = new Vector3(randomX, 1.5f, transform.position.z);
        while (elapsedTime < duration)
        {
            transform.position = Vector3.Lerp(startingPos, targetPos, elapsedTime / duration);
            elapsedTime += Time.deltaTime;
