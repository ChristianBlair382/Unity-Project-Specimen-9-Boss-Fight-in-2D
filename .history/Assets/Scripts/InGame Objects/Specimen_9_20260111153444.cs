...
public class Specimen_9 : MonoBehaviour
{
    ...
    {
        float projectileSpawnX = Random.Range(player.transform.position.x - 3, player.transform.position.x + 4);
        for (int i = 0; i < 3; i++)
        {
            Instantiate(ceilingProjectilePrefab, new Vector3(player.transform.position.x, 6, -2), Quaternion.identity);
            yield return new WaitForSeconds(0.5f);
        }
    }
