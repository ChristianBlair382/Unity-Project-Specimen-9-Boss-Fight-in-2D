...
public class Specimen_9 : MonoBehaviour
{
    ...
    void Update()
    {
        if (currentState == Specimen9State.Stunned)
        {
            ...
            if (stunTimer <= 0)
            {
                StartCoroutine(RecoveryTransition());
    ...
    public void Stun()
    {
        ...
        StartCoroutine(StunTransition());
    }
    private IEnumerator StunTransition()
    {
        //Upon being stunned, Specimen 9 will lerp to the ground and become succeptible to damage
        float elapsedTime = 0f;
        Vector3 startingPos = transform.position;
        Vector3 targetPos = new Vector3(transform.position.x, -1.5f, transform.position.z);
        while (elapsedTime < 1f)
        {
            transform.position = Vector3.Lerp(startingPos, targetPos, (elapsedTime / 1f));
            elapsedTime += Time.deltaTime;
            yield return null;
        }
        transform.position = targetPos;
    }
    private IEnumerator RecoveryTransition()
    {
        //Upon recovering from stun, Specimen 9 will lerp back to its original position
        float elapsedTime = 0f;
        Vector3 startingPos = transform.position;
